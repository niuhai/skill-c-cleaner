...
| 验证项 | 结果 |
|--------|------|
| 新发现 | DoubaoWork 0.72 GB、ZCode 0.35 GB；LobsterAI/CodeBuddy 当前很小但进入增长基线 |
| 签名补齐 | 新增 Doubao/iChat/GitHub Copilot；19 类 AF 配置覆盖旧 `ai_tools` 全部 detect path |
| 学习队列 | 只统计未被精确组件解释的余量；父子策略按路径并集去重；100 MB 以上 review 从 5 个收敛到 0 |
| 计量口径 | 明确为 C-located logical bytes；不把逻辑大小表述为 NTFS 实际分配或可释放量 |
| 最新实机值 | 29.01 GB logical；safe 4.47 GB；managed 3.82 GB；migration 21.67 GB |
| 性能 | 250,806 文件，0 skipped；最新热态原生 4.8 秒，总 AF 8.9 秒 |
| 结构测试 | 721 个配置/路径并集约束通过；最新 AF 报告 57 个核算/归属约束通过 |

本轮仍只读扫描和清理预览，没有执行删除、卸载、迁移、建链或环境变量修改。

---

## 测试批次 15：v7.0 AI 软件生命周期与实机学习闭环（2026-09-08）

用户反馈 AI 软件持续增长、传统缓存规则清理效果有限。AF 首轮把缺失的可选正则当成空正则，导致注册表和 AppX 归属扩散到所有应用，扫描约 80 秒；修复为空值显式归零并增加回归测试后，归属恢复正常。

| 验证项 | 结果 |
|--------|------|
| AI C 盘逻辑占用 | 27.78 GB，9 个实际存在的软件组；整个根目录仅作 inventory |
| 精确清理口径 | safe 4.31 GB；managed/confirm 3.52 GB；58 个去重精确路径 |
| 迁移口径 | 20.74 GB；WindowsApps 1.82 GB 只作系统设置 advisory，不混入候选总量 |
| 实机学习队列 | 首轮 19 个热点；扩展/插件/User/ModularData/runtime 等保守分类后剩 5 个 review |
| 性能 | 总扫描 11.5 秒；AF 原生枚举 7.4 秒；0 个跳过目录 |
| 清理预览 | 36 个 safe 精确路径、4.31 GB；Qoder 与 Codex 重定向根在子项枚举前跳过；0 个 PowerShell error |
| 测试 | 55 个 PowerShell 文件语法通过；AI 配置与空正则回归通过；统一删除门禁通过 |

本轮发布验证没有删除、迁移或卸载任何用户/系统数据。AF 基线和 JSON 报告只写入忽略的 `reports/` 目录。
