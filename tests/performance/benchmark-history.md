...
## v7.2.0：F→MX 测量复用（2026-09-08）

| 场景 | 修改前 | 修改后 | 证据 |
|------|--------|--------|------|
| `F,MX` 总耗时 | 65.3 秒 | 15.2–16.1 秒 | 同一台机器、同日、约 66.4 万文件，两次修改后复测 |
| F 全盘原生扫描 | 17.2 秒 | 14.2 秒 | 正常热态波动；修改未增加第二遍遍历 |
| MX 一级目录核算 | 48.0 秒 | 0.9–0.94 秒 | 复用 F 的 5 个大根；剩余 23 个根四路批量测量 |
| MS 厂商托管存储 | 无 | 0.1 秒 | WPS 18.50 GB + ESP-IDF dist 1.15 GB |

F 的 `PathTotals` 在同轮写入共享逻辑测量缓存；MX 不依赖 F 才能运行，单独执行时会批量测量一级目录。所有数值仍是逻辑字节，WPS/ESP-IDF 的实际分配值另由 `measure-space.ps1` 核验。

---

## v7.1.0：19 类 AI 覆盖与未解释字节（2026-09-08）

| 场景 | 耗时 | 证据 |
|------|------|------|
| AF 冷/有后台 I/O | 17.9–30.0 秒总耗时 | 19 类配置、约 25 万文件；用于观察上界，不伪装为稳定基线 |
| AF 热态复扫 | 8.9 秒总计、4.8 秒原生 | 250,806 文件；0 skipped；29.01 GB logical |
| 未解释字节队列 | 与 AF 同遍 | 已分类路径先折叠为非重叠并集再扣除；100 MB 以上 review 为 0 |

新增约 9,300 个 DoubaoWork/ZCode 等文件后，热态原生扫描仍低于 v7.0 的 7.4 秒记录；冷态受杀毒和并发应用 I/O 影响明显，因此报告同时保留每次 `elapsed_seconds`，不把单个最好成绩当成稳定承诺。

---

## v7.0.0：AI 软件生命周期一遍核算（2026-09-08）

| 场景 | 耗时 | 证据 |
|------|------|------|
| AF 错误归属基线 | 约 80 秒 | 空可选正则误匹配所有注册表/AppX 软件；结果不可用 |
| AF 修复后首轮 | 16.2 秒 | 27.77 GB，9 个应用组，0 个跳过目录 |
| AF 分类收敛复扫 | 11.5 秒（原生枚举 7.4 秒） | 27.78 GB；58 个精确清理路径；5 个 review 热点 |
| AF safe cleaner 预览 | 与 AF 同口径 4.31 GB | 36 个精确路径；0 error；junction 目标 fail closed |
| `analyze.ps1 -Fast`（含 AF） | 22.2 秒 | 65 条 findings、26 条 inventory；AF 原生枚举 7.3 秒 |

AF 使用同一遍 Win32 枚举聚合根、精确组件和一级热点；学习队列只扫描已知 AI 根的一级大项，不重新递归遍历 C 盘。
